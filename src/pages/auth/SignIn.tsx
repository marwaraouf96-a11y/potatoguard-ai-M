...
export default function SignIn() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(false)
    if (error) {
      setError(error.message.includes('Invalid login') ? 'Incorrect email or password. Please try again.' : error.message)
    } else {
    ...
    <div className="min-h-screen bg-[#F5F3E8] flex">
      <div className="hidden lg:flex lg:w-2/5 bg-[#12372A] flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-[#8BAE97] text-base leading-relaxed">
            Sign in to access your saved scan history, field cases, and research dashboard.
          </p>
          <div className="mt-8 p-4 bg-[#1F5040] rounded-xl border border-[#2A5540]">
            <p className="text-sm text-[#D4ECD9] font-medium mb-1">No account needed for basic use</p>
            <p className="text-xs text-[#8BAE97]">You can try the AI model and browse the research pages without signing in. An account is only needed to save scans or request expert review.</p>
          </div>
        </div>
        <p className="relative text-xs text-[#5A8A70]">AI-assisted screening only · Predictions are not clinical diagnoses</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <h1 className="text-2xl font-bold text-[#17221C] mb-1" style={{ fontFamily: 'Sora, sans-serif' }}>Sign In</h1>
          <p className="text-sm text-[#5A6B61] mb-6">Access your scans, dashboard, and saved field cases.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-sm font-medium text-[#17221C]">Password</label>
                <Link to="/auth/forgot-password" className="text-xs text-[#2F7D4A] hover:underline">Forgot password?</Link>
              </div>
              <div className="relative">
                <input
                  type={showPass ? 'text' : 'password'} autoComplete="current-password" value={password}
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} className="w-4 h-4 accent-[#2F7D4A]" />
              <span className="text-sm text-[#5A6B61]">Remember me on this device</span>
            </label>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 border border-[#D8D5C5] bg-white hover:bg-[#F5F3E8] text-[#17221C] font-medium rounded-lg transition-colors text-sm"
            >
          ...
          <p className="text-center text-sm text-[#5A6B61] mt-5">
            ...
            <Link to="/auth/signup" className="text-[#2F7D4A] font-semibold hover:underline">Sign Up</Link>
          </p>
          <p className="text-center text-sm text-[#5A6B61] mt-2">
            <Link to="/diagnose" className="text-[#2F7D4A] hover:underline">Try the model without an account →</Link>
          </p>
        </div>
      </div>
    </div>
